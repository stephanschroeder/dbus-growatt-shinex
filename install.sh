...
grep -qF  || echo "exec multilog t s153600 n2 /var/log/$SERVICE_NAME"
grep -qF "exec multilog t s153600 n2 /var/log/$SERVICE_NAME" $SCRIPT_DIR/service/log/run || echo exec multilog t s153600 n2 /var/log/$SERVICE_NAME >> $SCRIPT_DIR/service/log/run

grep -qF "$SCRIPT_DIR/install.sh" $filename || echo "/bin/bash $SCRIPT_DIR/install.sh" >> $filename
